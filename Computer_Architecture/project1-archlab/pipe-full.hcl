...
# 
# modified:
# 1. add iaddl instruction to this pipeline in this file. reason: in the previous ncopy.ys,the iaddl could replace two instructions namely mov $imm, %reg; addl %reg, %dst_reg,so after replacement,the speed can be improved.And after this addition,the register %edi can be freed and can make it convenient for ncopy.ys's modification.
# 
...
int d_srcA = [
	...
	1 : RNONE; # Don't need register tag
];
